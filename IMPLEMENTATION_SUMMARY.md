...
### 0.2 Backend secrets (`backend/.env`, gitignored)

Each teammate needs their own copy with the shared team's actual values (get these from whoever set them up — not committed to git). Copy `backend/.env.example` to `backend/.env` and fill in:
```properties
DB_URL=...
DB_USERNAME=...
DB_PASSWORD=...
